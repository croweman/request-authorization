...
function isAuthorizedMiddleware(getDataFunc) {

    return function(req, res, next) {

        var data = getDataFunc(req);

        if (!isAuthorized(req.headers['authorization'], data).isAuthorized) {
            res.status(401).end();
            return;
        }
...
module.exports = {
    ...
    isAuthorizedMiddleware: isAuthorizedMiddleware
};
...
RequestAuthorization.prototype.authorizationSchemes = [];
RequestAuthorization.prototype.init = function(schemes) {

    //this.authorizationSchemes = schemesBuilder.build(schemes);

    schemesBuilder.build(schemes).forEach(function(scheme) {
        RequestAuthorization.prototype.authorizationSchemes.push(scheme);
    });

    //RequestAuthorization.prototype.authorizationSchemes = schemesBuilder.build(schemes);
    console.log(RequestAuthorization.prototype.authorizationSchemes)
}

RequestAuthorization.prototype.generateAuthorizationHeader = function(options, data, timestampDate) {

    options = options || {};

    var scheme = findScheme(RequestAuthorization.prototype.authorizationSchemes, options.schemeName);
    var client = findClient(scheme, options.clientId);
...
RequestAuthorization.prototype.isAuthorized = function(authorizationHeader, data, timestampDate) {
    ...
    try {
        scheme = findScheme(RequestAuthorization.prototype.authorizationSchemes, parsedHeader.scheme);
    }
    catch (err) {
        result.error = err;
        return result;
...
RequestAuthorization.prototype.isAuthorizedMiddleware = function(getDataFunc) {

    return function(req, res, next) {

        var data = getDataFunc(req);

        if (!RequestAuthorization.prototype.isAuthorized(req.headers['authorization'], data).isAuthorized) {
            res.status(401).end();
            return;
        }
...
module.exports = new RequestAuthorization();*/
