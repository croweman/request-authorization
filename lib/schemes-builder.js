...
function validateClients(scheme) {
    ...
    for (var i = 0; i < clients.length; i++) {
        ...
        if (scheme.isHasher) {
            ...
            if (!relativeOrAbsolutePathToPublicKey || !(typeof relativeOrAbsolutePathToPublicKey === "string") || relativeOrAbsolutePathToPublicKey.length == 0)
                throw "relativeOrAbsolutePathToPublicKey is invalid";

            if (!checkFileExists(relativeOrAbsolutePathToPublicKey))
                throw "relativeOrAbsolutePathToPublicKey file path does not exist";

            if (!relativeOrAbsolutePathToPrivateKey || !(typeof relativeOrAbsolutePathToPrivateKey === "string") || relativeOrAbsolutePathToPrivateKey.length == 0)
                throw "relativeOrAbsolutePathToPrivateKey is invalid";

            if (!checkFileExists(relativeOrAbsolutePathToPrivateKey))
