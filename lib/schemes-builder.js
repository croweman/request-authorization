    ...
    for (var i = 0; i < schemes.length; i++) {
        ...
        for (var j = 0; j < this.availableEncryptors.length; j++) {
            ...
            if (this.availableEncryptors[j].name === schemeName) {
                ...
                requiresPassword = this.availableEncryptors[j].requiresPassword;
                break;
            }
        }
        ...
        var alias = scheme.alias;

        if (typeof alias !== 'undefined' && (!(typeof alias === "string") || alias.length == 0))
            throw 'invalid alias name has been provided';

        scheme.isHasher = isHasher;
        scheme.requiresPassword = requiresPassword;
...
function validateClients(scheme) {
    ...
    for (var i = 0; i < clients.length; i++) {
        ...
        if (scheme.isHasher) {
            ...
            if (scheme.requiresPassword && (!password || !(typeof password === "string") || password.length == 0)) {
